...
from tbdy_engine.features.resolver.live_smoke import (
    _norm_key,  # diagnostic-only normalized headers helper
...
def run_live_probe(out_dir: Path, *, target_story: str | None, preferred_output_case: str | None) -> dict[str, Any]:  # pragma: no cover - requires Windows/ETABS
    ...
    # Backward-compatible exact filenames requested by the sprint.
    name_map = {
    ...
    summary = {
        ...
        "target_story": target_story,
        "preferred_output_case": preferred_output_case,
        "tables": {alias: {"actual_table_name": r["actual_table_name"], "row_count": r["row_count"], "parser_status": r["parser_status"], "number_records": r["number_records"], "table_data_length": r["table_data_length"]} for alias, r in reports.items()},
    }
