...
from tbdy_engine.product.live_beam_column_minimum_compliance import run_live_beam_column_minimum_compliance
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the C14.1-P1 live beam/column minimum-compliance product")
    parser.add_argument("--live-etabs", action="store_true", help="Required explicit opt-in to attach to running ETABS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_live_beam_column_minimum_compliance(
        output_dir=args.out, element_type=args.element_type, story=args.story, section=args.section
    )
    print(f"engineering_status={result.get('engineering_status')}")
