"""VS-2 live RC component compliance pack -> one F0 regulatory execution.

This module is a bounded composition seam. It combines factual component
FeatureSnapshots and the authoritative M0 used-RC material population with the
five existing formal CheckSpecs. It does not acquire ETABS data and it does not
own engineering formulas.
"""
...
class VS2RcComponentIntegrationError(RuntimeError):
    """Bounded VS-2 integration failure."""
...
def load_rc_geometry_capture(feature_snapshot_path: Path) -> RcGeometryCaptureArtifact:
    ...
    if not isinstance(payload, Mapping):
        raise VS2RcComponentIntegrationError("FeatureSnapshot artifact root must be a mapping")
    raw_snapshots = payload.get("snapshots")
    if not isinstance(raw_snapshots, Sequence) or isinstance(
        raw_snapshots, (str, bytes, bytearray)
    ):
    ...
    snapshots = tuple(
        sorted(
            (
                ...
                and str(item.get("component_type") or "").strip().casefold() in {"beam", "column"}
            ),
    ...
    return RcGeometryCaptureArtifact(
        path=path,
        raw_bytes=raw_bytes,
        snapshots=snapshots,
        beam_snapshots=beams,
        column_snapshots=columns,
    )
...
def build_material_live_capture_epoch(
    *, model_fingerprint: str, source_bytes: bytes
) -> EvidenceEpoch:
...
def _trace_feature(snapshot: FeatureSnapshot) -> FeatureValue:
    evidence = tuple(
        ...
        for feature in (snapshot.features[name],)
...
def _context_bindings() -> tuple[F0EvidenceBinding, ...]:
    return (
        ...
        F0EvidenceBinding(
            ...
            source_key=COMPONENT_TRACE_FEATURE,
...
def component_bindings(component_type: str) -> tuple[F0EvidenceBinding, ...]:
    ...
    if kind == "beam":
        ...
        dimensions = (
            F0EvidenceBinding(
                source_location=EvidenceBindingSource.FEATURE_VALUE,
                source_key="column_width_mm",
                dependency_key=COLUMN_WIDTH_KEY,
                source_kind=DependencySourceKind.FACT,
            ...
            F0EvidenceBinding(
                ...
                dependency_key=COLUMN_DEPTH_KEY,
                source_kind=DependencySourceKind.FACT,
    ...
    return (*dimensions, *_context_bindings())
...
def _rectangular_source_proven(snapshot: FeatureSnapshot) -> bool | None:
    if snapshot.component_type.strip().casefold() != "column":
        return None
    observed = False
    for feature_name in ("column_width_mm", "column_depth_mm"):
        feature = snapshot.features.get(feature_name)
        if not isinstance(feature, FeatureValue):
            continue
        for evidence in feature.evidence:
            if evidence.evidence_status is not FeatureEvidenceStatus.FULL:
                continue
            observed = True
            if (
                ...
                return True
    return None if observed else None
    ...
    if kind == "beam":
        ...
        return (
            RuleScopeTarget(
                ...
                applicability_input=BeamMinWidthApplicabilityInput("beam", tbdy_7411_applies),
                **common,
            ),
            RuleScopeTarget(
                rule_id=BEAM_MIN_DEPTH_RULE_ID,
                applicability_input=Beam7411ApplicabilityInput(True, tbdy_7411_applies),
                **common,
            ),
            RuleScopeTarget(
    ...
    if kind == "column":
        return (
            RuleScopeTarget(
                ...
                applicability_input=ColumnMinDimensionApplicabilityInput(
                    is_column=True,
                    is_rectangular_section=_rectangular_source_proven(snapshot),
                ),
                **common,
            ),
        )
...
def _material_definition_payload(
    population: UsedRcMaterialPopulation, material: UsedMaterialDefinition
) -> Mapping[str, object]:
...
def _material_fact_ref(
    population: UsedRcMaterialPopulation, material: UsedMaterialDefinition
) -> str:
    digest = hashlib.sha256(
        _canonical_json_bytes(_material_definition_payload(population, material))
    ).hexdigest()
    return f"material-fact:sha256:{digest}"


def _material_authority_id(
    *, epoch: EvidenceEpoch, material_id: str, dependency_key: str, factual_ref: str
) -> str:
    digest = hashlib.sha256(
...
def material_authorities(
    *,
    ...
    if not material.is_concrete:
        raise VS2RcComponentIntegrationError("material authority requires factual concrete material")
    if material.concrete_strength_status is not ConcreteStrengthFactStatus.RESOLVED:
        raise VS2RcComponentIntegrationError("material authority requires resolved canonical fck")
    if material.canonical_fck_mpa is None:
        raise VS2RcComponentIntegrationError("resolved material strength is missing canonical fck")
    ...
    provenance = tuple(
        dict.fromkeys(
            (
                ...
                f"source:{epoch.source_fingerprint}",
    ...
    return (
        ExternalDependencyAuthority(
            authority_id=_material_authority_id(
                epoch=epoch,
                material_id=material.material_id,
                dependency_key=FCK_KEY.value,
                factual_ref=factual_ref,
            ),
            key=FCK_KEY,
            source_kind=DependencySourceKind.FACT,
        ...
        ExternalDependencyAuthority(
            authority_id=_material_authority_id(
                epoch=epoch,
                material_id=material.material_id,
                dependency_key=MATERIAL_EVIDENCE_TRACE_KEY.value,
                factual_ref=factual_ref,
            ),
...
def _finding_provenance(
    *, instance, geometry_epoch: EvidenceEpoch, material_epoch: EvidenceEpoch
) -> tuple[str, ...]:
    epoch = material_epoch if instance.grain is Grain.MATERIAL_DEFINITION else geometry_epoch
    return tuple(
        item
        for item in (epoch.epoch_id, epoch.model_fingerprint, epoch.source_fingerprint)
        if item
    )
...
def run_live_rc_component_f0_pack(
    *,
    ...
    tbdy_7411_applies = validate_tbdy_7411_applies(tbdy_7411_applies)
    if geometry_epoch.origin is not EvidenceEpochOrigin.LIVE_CAPTURE:
        raise VS2RcComponentIntegrationError("geometry epoch must be LIVE_CAPTURE")
    if material_epoch.origin is not EvidenceEpochOrigin.LIVE_CAPTURE:
        raise VS2RcComponentIntegrationError("material epoch must be LIVE_CAPTURE")
    ...
    prepared = tuple(
        sorted(
            (prepare_component_snapshot(item) for item in snapshots),
            key=lambda item: (item.component_type.casefold(), item.component_id),
        )
    )
    ...
    component_authorities = tuple(
        authority
        for snapshot in prepared
        for authority in build_component_f0_authorities(
    ...
    if material_supported:
        concrete_materials = tuple(
            sorted(
                material_population.used_concrete_material_definitions,
                key=lambda item: item.material_id,
            )
        )
        material_targets = tuple(material_target(item) for item in concrete_materials)
        material_external = tuple(
            authority
            for item in concrete_materials
            for authority in material_authorities(
                epoch=material_epoch,
                population=material_population,
                material=item,
            )
        )
    else:
    ...
    compile_inputs = build_f0_compile_inputs(
        rule_targets=(*targets, *material_targets),
        external_authorities=(*component_authorities, *material_external),
    )
    program = RegulatoryCompiler.compile(VS2_RC_REGISTRY, compile_inputs)
    ...
    check_findings = tuple(
        ...
        for finding in (
            build_finding_from_check_result(
                ...
                provenance_refs=_finding_provenance(
    ...
    closure_findings = tuple(
        ...
        for finding in (
            build_finding_from_rule_closure(
                ...
                outcome=outcome,
                provenance_refs=_finding_provenance(
                    instance=outcome.compiled_record_ref,
                    geometry_epoch=geometry_epoch,
                    material_epoch=material_epoch,
                ),
            ),
    ...
    return LiveRcComponentPackRun(
        ...
        authorities=tuple((*component_authorities, *material_external)),
