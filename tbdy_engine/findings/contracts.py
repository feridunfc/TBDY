...
def _validate_source_coherence(
    *,
...
) -> tuple[str, str, str | None, RuleInstanceId | None]:
    if not isinstance(source_kind, FindingSourceKind):
        raise TypeError("source_kind must be FindingSourceKind")
    ...
    if source_kind in {FindingSourceKind.CHECK_RESULT, FindingSourceKind.RULE_CLOSURE}:
        if rule_instance_ref is None:
            raise ValueError(f"{source_kind.value} requires rule_instance_ref")
        if scope_ref != rule_instance_ref.scope_ref:
            raise ValueError("scope_ref must match rule_instance_ref.scope_ref")
        if direction != rule_instance_ref.direction:
            raise ValueError("direction must match rule_instance_ref.direction")
        prefix = (
            "check-result:"
            if source_kind is FindingSourceKind.CHECK_RESULT
            else "rule-closure:"
        )
        expected_source_ref = f"{prefix}{rule_instance_ref.value}"
        if source_ref != expected_source_ref:
            raise ValueError("source_ref is inconsistent with canonical stored semantic fields")
        return source_ref, scope_ref, direction, rule_instance_ref

    if rule_instance_ref is not None:
        raise ValueError("ANALYSIS_BASIS_COMPATIBILITY requires rule_instance_ref=None")
    if direction is None:
        raise ValueError("ANALYSIS_BASIS_COMPATIBILITY requires direction")
    prefix = "analysis-basis-compatibility:"
    if not source_ref.startswith(prefix):
        raise ValueError(
            "ANALYSIS_BASIS_COMPATIBILITY source_ref must use "
            "analysis-basis-compatibility:<id> form"
        )
    _canonical_text(
        source_ref.removeprefix(prefix),
        "analysis_basis_compatibility_id",
    )
    return source_ref, scope_ref, direction, rule_instance_ref
...
def _finding_identity(
    *,
    ...
    source_ref, scope_ref, direction, rule_instance_ref = _validate_source_coherence(
