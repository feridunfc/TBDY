...
_ASSIGNMENT_SECTION_COLUMN_CANDIDATES = ("SectProp", "Section", "SectionName", "PropName")
...
class _EtabsComGeometryProvider:
    ...
    def iter_geometry_rows(self) -> Sequence[Mapping[str, object]]:
        rows, _diagnostics = self._resolved_rows_and_diagnostics()
        return rows

    def iter_geometry_diagnostics(self) -> Sequence[LiveGeometryProbeDiagnostic]:
        _rows, diagnostics = self._resolved_rows_and_diagnostics()
        return diagnostics

    def _resolved_rows_and_diagnostics(self) -> tuple[tuple[Mapping[str, object], ...], tuple[LiveGeometryProbeDiagnostic, ...]]:
        ...
        assignment_rows = _read_live_table_rows(database_tables, self.mapping.assignment_table_key)
        property_rows = _read_live_table_rows(database_tables, self.mapping.property_table_key)
