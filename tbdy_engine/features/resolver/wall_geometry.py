"""Generic canonical wall-geometry fact resolver for P2.10 Pack A and later packs."""
...
_SUPPLEMENTAL_FACTS = tuple(WALL_GEOMETRY_SUPPLEMENTAL_FEATURE_DEFINITIONS)
...
class WallGeometryFactResolver:
    """Resolve canonical factual wall geometry without engineering verdicts."""
    ...
    def __init__(
        ...
        self.feature_defs = {**base_features, **dict(WALL_GEOMETRY_FEATURE_DEFINITIONS)}
    ...
    def build_snapshots(
        ...
        *,
        provided_features_by_wall: Mapping[str, Mapping[str, FeatureValue]] | None = None,
    ...
    def resolve_record(
        ...
        *,
        provided_features: Mapping[str, FeatureValue] | None = None,
    ) -> FeatureSnapshot:
        if record.classification_status != WallInventoryStatus.STRUCTURAL_WALL_CANDIDATE:
        ...
        if unknown:
            raise ValueError(
                "Only factual supplemental wall context may be supplied externally: "
                + ", ".join(sorted(unknown))
            )
        ...
        for feature_id in _SUPPLEMENTAL_FACTS:
            facts[feature_id] = (
                self._provided_fact(feature_id, supplied[feature_id])
                if feature_id in supplied
                else self._missing(feature_id, record, "Canonical factual context has not been resolved")
            )

        return FeatureSnapshot(
            component_type="wall",
            component_id=str(record.wall_object_id),
    ...
    def _wall_thickness_fact(self, record: WallInventoryRecord) -> FeatureValue:
        feature_id = "wall_thickness_mm"
        table = self.thickness_table
        if table is None:
        ...
        row = rows[0]
        column, raw = self._field(row, self._aliases(feature_id))
        if column is None:
    ...
    def _wall_length_fact(self, record: WallInventoryRecord) -> FeatureValue:
        """Resolve lw from verified Pier Section Properties using exact story+pier identity."""
        feature_id = "wall_length_mm"
        table = self.length_table
        if table is None:
            return self._missing(feature_id, record, "Verified pier-section wall-length table is unavailable")
        story = self._identifier(record.story)
        pier = self._identifier(record.pier_assignment)
        if story is None or pier is None:
            return self._missing(
                feature_id,
                record,
                "Wall length requires exact story and pier identity; no unrelated total length is substituted",
            )
        rows = tuple(
            row
            for row in table.rows
            if self._identifier(self._field(row, _STORY_ALIASES)[1]) == story
            and self._identifier(self._field(row, _PIER_ALIASES)[1]) == pier
        )
        if not rows:
    ...
    def _normalized_length(
        ...
        resolved = (
            ...
            and normalized.provenance.get("normalization_status") == "RESOLVED"
        )
        ...
        evidence = FeatureEvidence(
            ...
            source_row=self._source_payload(record, table, row, normalized.as_dict()),
            raw_value=raw,
            normalized_value=normalized.normalized_value if resolved else None,
            unit="mm",
            resolver=_RESOLVER,
            reason=reason,
        )
        diagnostics = [] if resolved else [
            FeatureDiagnostic(
                ...
                reason or "Unit normalization unavailable",
                {"feature": feature_id, "source_table": table.table_key},
            )
        ]
        return FeatureValue(
            feature_name=feature_id,
            value=normalized.normalized_value if resolved else None,
            unit="mm",
    ...
    def _unresolved(
        ...
        evidence = FeatureEvidence(
            ...
            source_table=(self.feature_defs.get(feature_id, {}).get("source") or {}).get("table_key"),
            source_row=self._source_payload(record, None, None, None),
            unit=unit,
            resolver=_RESOLVER,
            reason=reason,
        )
    ...
    def _source_payload(
        ...
        return {
            ...
            "unit_context": dict(table.units) if table is not None else {},
