...
is permitted and no design row disappears. A design row may be promoted only
when its component and PMM combo name bind to exactly one ELIGIBLE
``ColumnComboEligibilityProjection`` from the same model fingerprint and
EvidenceEpoch.
"""
...
from tbdy_engine.design.columns.column_concrete_design_evidence_authority import DesignComboIdentity
