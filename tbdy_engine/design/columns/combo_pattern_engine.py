...
ETABS definition and the factual case types of all flattened constituents match
an explicitly implemented pattern.
...
class ComboPatternError(ValueError):
    """Raised when the factual classifier inputs are malformed."""
...
def classify_combo_pattern(
    *,
    combo_name: str,
    combo_type: str,
    constituents: Sequence[LinearComboConstituent],
    ...
    * LINEAR_ADD only;
    * already-flattened LOAD_CASE constituents only;
    * LinStatic and LinRespSpec case types only.
    ...
    missing = tuple(sorted({term.name for term in terms if term.name not in case_types}))
    if missing:
