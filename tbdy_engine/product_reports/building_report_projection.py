...
* copied exactly from canonical upstream data,
...
_DOMAIN_SPECS = (
    ("model-inventory", "Model inventory", ("MODEL_INVENTORY",), "Canonical inventory contributions."),
    ("loads-mass-cases", "Loads / mass / cases / design combinations", ("LOADS_AND_MASS", "LOAD_CASE", "DESIGN_COMBINATION"), "Canonical load, mass-source, case, and combination contributions."),
    ("global-analysis", "Global analysis", ("GLOBAL_ANALYSIS",), "Canonical modal, base-reaction, drift, irregularity, and related global-analysis contributions."),
    ("structural-system", "Structural-system qualification", ("STRUCTURAL_SYSTEM",), "Canonical structural-system qualification contributions."),
    ("beams", "Beams", ("BEAM",), "Canonical beam contributions."),
    ("columns", "Columns", ("COLUMN",), "Canonical column contributions."),
    ("scwb-joints", "SCWB / joints", ("SCWB_JOINT", "JOINT"), "Canonical joint and SCWB contributions."),
    ("walls", "Walls", ("WALL",), "Canonical wall contributions."),
    ("slab-diaphragm", "Slab / diaphragm", ("SLAB", "DIAPHRAGM", "SLAB_DIAPHRAGM"), "Canonical slab and diaphragm contributions."),
    ("foundation-geotechnical", "Foundation / geotechnical", ("FOUNDATION", "GEOTECHNICAL"), "Canonical foundation and geotechnical contributions."),
)
...
def _sorted_contributions(model: BuildingReportModel):
    return tuple(sorted(model.contributions, key=lambda item: ReportContributionRef.from_contribution(item).sort_key))
...
def _coverage_display(summary: dict[str, object]) -> list[dict[str, object]]:
    ...
    for key in _COVERAGE_LABELS:
        if key in summary:
            result.append({"canonical_key": key, "label": _COVERAGE_LABELS[key], "value": summary[key]})
    for key in sorted(set(summary) - set(_COVERAGE_LABELS)):
        result.append({"canonical_key": key, "label": "Additional canonical coverage metric", "value": summary[key]})
    return result


def _analysis_basis_summary(model: BuildingReportModel) -> dict[str, object]:
    instance_ids = tuple(sorted(item.value for item in model.reconciliation.reanalysis_required_instance_ids))
    return {"reanalysis_required_count": len(instance_ids), "reanalysis_required_instance_ids": list(instance_ids)}
...
def _projected_contributions(model: BuildingReportModel) -> list[dict[str, object]]:
    ...
    for contribution in _sorted_contributions(model):
        ...
        if not source_refs:
            raise ReportProjectionIntegrityError("canonical contribution has no report binding source identity: " + contribution_ref.value)
...
def _kind_facets(model: BuildingReportModel) -> list[dict[str, object]]:
    ...
    return [{"contribution_kind": kind, "count": counts[kind]} for kind in sorted(counts)]
...
def _component_facets(model: BuildingReportModel) -> list[dict[str, object]]:
    ...
    for contribution in model.contributions:
        ...
        grouped.setdefault(key, []).append(ReportContributionRef.from_contribution(contribution).value)

    def sort_key(item: tuple[tuple[str | None, str | None], list[str]]):
        (component_type, component_id), _ = item
        return (component_type is None, component_type or "", component_id is None, component_id or "")

    return [
        {"component_type": component_type, "component_id": component_id, "contribution_count": len(refs), "contribution_refs": sorted(refs)}
        for (component_type, component_id), refs in sorted(grouped.items(), key=sort_key)
    ]


def _attention_items(contributions: list[dict[str, object]]) -> list[dict[str, object]]:
...
def _presentation_domains(contributions: list[dict[str, object]]) -> list[dict[str, object]]:
    token_to_domain: dict[str, str] = {}
    for domain_id, _, tokens, _ in _DOMAIN_SPECS:
        for token in tokens:
            token_to_domain[token] = domain_id
    grouped: dict[str, list[str]] = {domain_id: [] for domain_id, _, _, _ in _DOMAIN_SPECS}
    grouped["appendices"] = []
    for item in contributions:
        ref = str(item["contribution_ref"])
        component_type = item.get("component_type")
        domain_id = token_to_domain.get(component_type, "appendices") if isinstance(component_type, str) else "appendices"
        grouped[domain_id].append(ref)
    result = [
        {"domain_id": domain_id, "label": label, "canonical_component_types": list(tokens), "description": description, "contribution_refs": list(grouped[domain_id]), "contribution_count": len(grouped[domain_id])}
    ...
    result.append({
        ...
        "description": "Canonical contributions without an exact supported presentation-domain component_type token. No domain is inferred from names or identifiers.",
        "contribution_refs": list(grouped["appendices"]),
...
def _report_context(model: BuildingReportModel) -> dict[str, object]:
    return {"data_classification": _basis_value(model, "report_data_classification"), "report_phase": _basis_value(model, "report_phase")}


def _action_register(model: BuildingReportModel) -> dict[str, list[object]]:
    reconciliation = model.reconciliation.as_dict()
    keys = ("required_action_finding_ids", "missing_action_finding_ids", "duplicate_action_finding_ids", "orphan_action_binding_finding_ids")
...
def _report_input_gaps() -> list[dict[str, str]]:
    return [
        {"gap_id": "PROJECT_IDENTITY_DETAIL", "status": "REPORT_INPUT_GAP", "needed_for": "Professional cover / project identity", "detail": "BuildingReportModel has project_id/title and ProjectBasisLedger, but no typed client, address, engineer, revision, or issue metadata contract."},
        {"gap_id": "EXPLICIT_ENGINEERING_DOMAIN", "status": "REPORT_INPUT_GAP", "needed_for": "Stable engineering-domain navigation", "detail": "SliceReportContribution has no first-class engineering_domain field. UR-2 groups only exact component_type tokens and leaves unknown tokens in appendices without fuzzy inference."},
        {"gap_id": "TYPED_RESULT_CONTEXT", "status": "REPORT_INPUT_GAP", "needed_for": "Story / direction / case-combo / demand-capacity summary", "detail": "Story, direction, case/combo, demand, capacity, and ratio are not first-class typed contribution slots; they may exist only as canonical ReportField/ReportTable data and are rendered without reinterpretation."},
        {"gap_id": "ACTION_REMEDIATION_RECORDS", "status": "REPORT_INPUT_GAP", "needed_for": "Required actions / remediation register", "detail": "Canonical reconciliation can expose action finding identities, but the report contract does not yet carry structured remediation text, owner, required input, closure state, or reanalysis flag."},
        {"gap_id": "MODEL_EPOCH_TOP_LEVEL", "status": "REPORT_INPUT_GAP", "needed_for": "Cover-level model / evidence epoch identity", "detail": "BuildingReportModel has SourceManifest provenance but no typed top-level model fingerprint / EvidenceEpoch presentation field."},
    ]
...
def _validate_view_accounting(model: BuildingReportModel, view: ReportView) -> None:
    ...
    if missing:
        raise ReportProjectionIntegrityError(f"mandatory bound contribution excludes canonical {view.value} view: " + ", ".join(sorted(missing)))
...
def _presentation_contract() -> dict[str, object]:
    return {
        ...
        "domain_inference_from_display_text_allowed": False,
    }
...
class BuildingReportProjection:
    ...
    def _identity_dict(self) -> dict[str, object]:
        return {"schema_version": "building_report_projection.ur_1b.v1", "artifact_type": "BUILDING_REPORT_PROJECTION", "view": self.view.value, "report_id": self._model.report_id, "project_id": self._model.project_id, "title": self._model.title, "report_integrity_status": self._model.report_integrity_status}

    def as_dict(self) -> dict[str, object]:
        basis_refs = tuple(sorted(self._model.reconciliation.analysis_basis_refs, key=lambda item: item.instance_id.value))
        ...
        payload["coverage_display"] = _coverage_display(coverage_summary)
        ...
        payload["report_context"] = _report_context(self._model)
        payload["attention_items"] = _attention_items(contributions)
        payload["presentation_domains"] = _presentation_domains(contributions)
        payload["action_register"] = _action_register(self._model)
        payload["report_input_gaps"] = _report_input_gaps()
        payload["presentation_contract"] = _presentation_contract()

        if self.view is ReportView.ENGINEERING:
            payload["analysis_basis_warnings"] = [_analysis_basis_row(item) for item in basis_refs if item.status is not AnalysisBasisStatus.MATCH]
            return payload
        if self.view is ReportView.AUDIT:
            ...
            payload["report_bindings"] = [{"source_ref": item.source_ref, "contribution_ref": item.contribution_ref.value} for item in self._model.report_bindings]
...
def project_building_report_view(model: BuildingReportModel, view: ReportView) -> BuildingReportProjection:
    ...
    if not isinstance(view, ReportView):
        raise ReportProjectionIntegrityError("Unified reporting supports only ReportView.ENGINEERING and ReportView.AUDIT")
    return BuildingReportProjection(view=view, _model=model)


__all__ = ["BuildingReportProjection", "ReportProjectionIntegrityError", "ReportView", "project_building_report_view"]
