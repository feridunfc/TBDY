"""Internal C14.1-P1 product runner."""
...
from tbdy_engine.product._minimum_compliance_checks import (
    _column_derived_results, _blocked_result, _copy_result,
    _clear_span_candidate, _web_trigger_result, _feature_value, _feature_evidence,
)
from tbdy_engine.product._minimum_compliance_record import _record
...
def run_live_beam_column_minimum_compliance(*, output_dir: Path, element_type: str | None = None, story: str | None = None, section: str | None = None, attach_runner: AttachRunner = attach_to_running_etabs, source_loader: SourceLoader | None = None) -> Mapping[str, object]:
    ...
    classifications = _classify_inventory(
        inventory=inventory, section_rows=section_rows, material_rows=material_rows,
        snapshot_by_component=snapshot_by_component, source_diagnostics=source_diagnostics,
    )
    engine = MinimalCheckEngine(check_definitions=_load_catalog())
    ...
    for snapshot in sorted(snapshots, key=_snapshot_key):
        ...
        if component_type == "beam":
            ...
            span_result = _blocked_result(
                snapshot, "beam_span_depth_ratio", "BEAM_SPAN_DEPTH_RULE_NOT_LOCKED",
                "The governing span-depth rule and clear-span semantics are not locked.",
                code_ref=None, evidence=(candidate,) if candidate else (),
            )
            material_result = _blocked_result(
                snapshot, "beam_material_min_concrete_strength", "MATERIAL_MINIMUM_RULE_NOT_LOCKED",
                "The 25 MPa candidate minimum has no locked TBDY clause in this sprint.",
                code_ref=None, value=_feature_value(snapshot, "concrete_fck_mpa"), limit=25.0,
                unit="MPa", evidence=_feature_evidence(snapshot, "concrete_fck_mpa"),
            )
            ...
            if "column_geometry_min_dimension" in inputs:
                result = engine.run_check(
                    "column_geometry_min_dimension", inputs["column_geometry_min_dimension"].snapshot,
                    inputs["column_geometry_min_dimension"].coverage,
                )
                check_records.append(_record(result))
            check_records.extend(_record(result) for result in _column_derived_results(snapshot))
            material_result = _blocked_result(
                snapshot, "column_material_min_concrete_strength", "MATERIAL_MINIMUM_RULE_NOT_LOCKED",
                "The 25 MPa candidate minimum has no locked TBDY clause in this sprint.",
                code_ref=None, value=_feature_value(snapshot, "concrete_fck_mpa"), limit=25.0,
                unit="MPa", evidence=_feature_evidence(snapshot, "concrete_fck_mpa"),
            )
    ...
    for item in classifications:
        if item["status"] != "SUPPORTED":
            result = CheckResult(
                check_id="minimum_compliance_scope", component=str(item["unique_name"]),
                component_type=str(item["element_type"]), story=item.get("story"), section=item.get("section"),
                status=CheckStatus(str(item["status"])), evaluation_level=EvaluationLevel.NO_DATA,
                evidence=[item], messages=(str(item["reason"]),), code_ref=None,
            )
            check_records.append(_record(result))
    check_records.sort(key=_check_key)
    adapter_diagnostics.sort(key=_diagnostic_key)
    snapshots.sort(key=_snapshot_key)
    tables = _build_report_tables(
        inventory=inventory, classifications=classifications, snapshots=snapshots,
        check_records=check_records, diagnostics=adapter_diagnostics,
        connectivity=connectivity, offsets=offsets, unit_evidence=unit_evidence,
    )
...
__all__ = ["run_live_beam_column_minimum_compliance"]
