...
def _write_minimal_workbook(path: Path, *, include_drifts: bool = False) -> None:
    with pd.ExcelWriter(path) as writer:
        ...
        ]).to_excel(writer, sheet_name="Frame Section Property Definitions - Concrete Rectangular", index=False)
        pd.DataFrame([
            {"UniqueName": "B101", "Label": "B101", "Story": "5"},
        ]).to_excel(writer, sheet_name="Beam Object Connectivity", index=False)
        pd.DataFrame([
            {"UniqueName": "C101", "Label": "C101", "Story": "5"},
        ]).to_excel(writer, sheet_name="Column Object Connectivity", index=False)
        pd.DataFrame([
        ...
        ]).to_excel(writer, sheet_name="Frame Assignments - Section Properties", index=False)
        if include_drifts:
...
def test_workbook_snapshot_runs_existing_archx_checks(tmp_path):
    ...
    result = run_archx_checks(build_snapshot_from_etabs_workbook(workbook), run_id="workbook-run")
...
def test_story_drift_no_data_without_drift_limit(tmp_path):
    ...
    result = run_archx_checks(build_snapshot_from_etabs_workbook(workbook), check_ids=["story_drift"], run_id="drift-no-data")
...
def test_cli_etabs_workbook_writes_json(tmp_path):
    ...
    completed = subprocess.run(
        [sys.executable, "-m", "tbdy_engine.archx.cli", "--etabs-workbook", str(workbook), "--drift-limit", "0.02", "--out", str(output)],
        check=False,
        capture_output=True,
        text=True,
    )
...
def test_missing_tables_do_not_crash(tmp_path):
    ...
    with pd.ExcelWriter(workbook) as writer:
        pd.DataFrame([{"Name": "S1", "t2": 300, "t3": 500}]).to_excel(
            writer, sheet_name="Frame Section Property Definitions - Concrete Rectangular", index=False
        )
