            ...
            "feature_catalog.yaml": {
                "features": {
                    "wall_thickness_mm": {
                        ...
                        "source": {"table_key": "wall_section_data", "field_aliases": ["ThickBot"]},
                    },
                    "wall_length_mm": {
                        "element_type": "wall",
                        "unit": "mm",
                        "semantic_role": "GEOMETRY",
                        "source": {"table_key": "wall_section_data", "field_aliases": ["WidthBot"]},
                    },
                    "story_height_mm": {
                        "element_type": "story",
                        "unit": "mm",
                        "semantic_role": "GEOMETRY",
                        "source": {"table_key": "story_definitions", "field_aliases": ["Height"]},
                    },
                }
            },
...
def _units(length_unit: str = "m", *, resolved: bool = True):
    return {
        "source": "fixture_etabs_unit_context",
        "force_unit": "kN",
        "length_unit": length_unit,
        "temperature_unit": "C",
        "unit_query_status": "RESOLVED" if resolved else "PARTIAL",
...
def _tables(
    *,
    ...
    length_unit="m",
    resolved_units=True,
    property_name="WALL-P1",
    piers=("P1",),
    duplicate_pier_row=False,
):
    pier_rows = [
        {"Story": "L1", "Pier": pier, "Width": width, "Thickness": thickness, "Material": "C35"}
        for pier in piers
    ]
    if duplicate_pier_row and pier_rows:
        pier_rows.append(dict(pier_rows[0]))
    return {
        "wall_section_properties": CanonicalTable(
            ...
            columns=("Name", "Material", "Thickness"),
            rows=({"Name": property_name, "Material": "C35", "Thickness": thickness},),
            units=_units(length_unit, resolved=resolved_units),
            source="FAKE_PROVIDER",
        ),
        "pier_section_properties": CanonicalTable(
            table_key="pier_section_properties",
            actual_table_name="Pier Section Properties",
            columns=("Story", "Pier", "Width", "Thickness", "Material"),
            rows=tuple(pier_rows),
            units=_units(length_unit, resolved=resolved_units),
            source="FAKE_PROVIDER",
        ),
        "story_definitions": CanonicalTable(
            table_key="story_definitions",
...
def _inventory(count=1, property_name="WALL-P1", *, include_piers=True):
    area_rows = [
        {
            "UniqueName": f"A{i}",
            "Label": f"W{i}",
            "Story": "L1",
            "SectionProperty": property_name,
            "PropertyType": "Wall",
        }
    ...
    pier_rows = (
        [
            {"UniqueName": f"A{i}", "Story": "L1", "Label": f"W{i}", "Pier": f"P{i}"}
            for i in range(1, count + 1)
        ]
        if include_piers
        else []
    ...
    return build_wall_inventory(
        ...
        wall_property_rows=({"Name": property_name, "Material": "C35"},),
        pier_assignment_rows=pier_rows,
    )


def _provided_feature(name, value, unit="", role="APPLICABILITY_FACT"):
    evidence = FeatureEvidence(
        evidence_status=FeatureEvidenceStatus.FULL,
        source_table="canonical_pack_a_context",
        actual_table_name="canonical_pack_a_context",
        source_column=name,
        source_row={"feature": name, "value": value},
        raw_value=value,
        normalized_value=value,
        unit=unit,
        resolver="test_canonical_context",
    )
    return FeatureValue(
        feature_name=name,
        value=value,
...
def _provided_context(*, basement=False, restrained=True):
    return {
        "wall_is_basement": _provided_feature("wall_is_basement", basement),
        "wall_body_classification": _provided_feature(
            "wall_body_classification", "RECTANGULAR_BODY", role="GEOMETRY_CLASSIFICATION"
        ),
        "unrestrained_plan_length_mm": _provided_feature(
            "unrestrained_plan_length_mm", 7500.0, "mm", "GEOMETRY"
        ),
        "wall_geometry_classification": _provided_feature(
            "wall_geometry_classification", "RECTANGULAR_WALL", role="GEOMETRY_CLASSIFICATION"
        ),
        "wall_both_ends_laterally_restrained": _provided_feature(
            "wall_both_ends_laterally_restrained", restrained, role="TOPOLOGY"
        ),
    }
...
def _snapshots(
    *,
    ...
    inventory = _inventory(count, property_name, include_piers=include_piers)
    if tables is None:
        tables = _tables(property_name=property_name, piers=tuple(f"P{i}" for i in range(1, count + 1)))
    resolver = WallGeometryFactResolver(_Bundle(), tables)
    provided = {
        record.wall_object_id: (context_factory(record) if context_factory else _provided_context())
        for record in inventory.records
        if record.wall_object_id
    }
...
def test_wall_inventory_identity_survives_all_five_checks_for_each_wall():
    ...
    wall_ids = {record.wall_object_id for record in inventory.records if record.wall_object_id}
    assert {snapshot.component_id for snapshot in snapshots} == wall_ids
...
def test_source_units_normalize_before_check_engine_and_no_magnitude_heuristic():
    _, snapshots = _snapshots(
        tables=_tables(thickness="0.4", width="2.4", story_height="3.2", length_unit="m")
    )
    ...
    assert snapshot.features["wall_length_mm"].evidence[0].source_table == "pier_section_properties"
    ...
    _, mm = _snapshots(
        tables=_tables(thickness="0.4", width="2400", story_height="3200", length_unit="mm")
    )
    assert mm[0].features["wall_thickness_mm"].value == pytest.approx(0.4)
    assert normalize_length_to_mm(
        "0.4", raw_unit="cm", unit_context_trusted=True
    ).normalized_value == pytest.approx(4.0)


def test_wall_length_requires_exact_pier_section_evidence_and_no_unrelated_fallback():
    _, snapshots = _snapshots(include_piers=False)
    length = snapshots[0].features["wall_length_mm"]
    assert length.status == FeatureValueStatus.MISSING
...
def test_ambiguous_pier_section_length_fails_closed():
    _, snapshots = _snapshots(tables=_tables(duplicate_pier_row=True))
    assert snapshots[0].features["wall_length_mm"].status == FeatureValueStatus.PARTIAL
    result = next(r for r in run_wall(snapshots) if r.check_id == WALL_GEOM_DEFINITION_LW_BW_GE6)
    assert result.status == CheckStatus.BLOCKED


def test_untrusted_unit_context_fails_closed_and_formal_results_block():
...
def test_missing_global_basement_fact_blocks_all_five_checks():
    def context(_record):
        data = _provided_context()
        data.pop("wall_is_basement")
        return data
...
def test_engineering_derived_special_branch_flag_is_rejected_from_feature_snapshot():
    def context(_record):
        data = _provided_context()
        data["wall_special_branch_7_6_1_3_applies"] = _provided_feature(
            "wall_special_branch_7_6_1_3_applies", False
        )
        return data

    with pytest.raises(ValueError, match="Only factual supplemental wall context"):
        _snapshots(context_factory=context)


def test_h16_and_250_are_blocked_until_7613_applicability_is_derivable():
    _, snapshots = _snapshots()
    results = {result.check_id: result for result in run_wall(snapshots)}
    ...
    assert results[WALL_GEOM_UNRESTRAINED_THICKNESS_GE_L30].status == CheckStatus.OK
    assert results[WALL_GEOM_RESTRAINED_LEG_THICKNESS].status == CheckStatus.OK
...
def test_basement_applicability_comes_from_fact_not_property_name():
    ...
    assert all(result.status == CheckStatus.OUT_OF_SCOPE for result in run_wall(snapshots))

    name = "BsmntWall_40cm"
    _, snapshots = _snapshots(
        tables=_tables(property_name=name),
        property_name=name,
        context_factory=lambda _: _provided_context(basement=False),
    )
    definition = next(
        result for result in run_wall(snapshots) if result.check_id == WALL_GEOM_DEFINITION_LW_BW_GE6
    )
...
def test_property_identity_join_is_exact_and_case_sensitive():
    ...
    assert all(result.status == CheckStatus.BLOCKED for result in run_wall(snapshots))


def test_unknown_both_ends_restraint_blocks_restrained_leg_check():
    def context(_record):
        data = _provided_context()
        data.pop("wall_both_ends_laterally_restrained")
        return data

    _, snapshots = _snapshots(context_factory=context)
    result = next(
        result for result in run_wall(snapshots) if result.check_id == WALL_GEOM_RESTRAINED_LEG_THICKNESS
    )
    assert result.status == CheckStatus.BLOCKED


def test_executable_pack_a_formulas_are_made_only_by_check_engine():
    _, _, run = _run()
    ...
    assert results[WALL_GEOM_BODY_THICKNESS_GE_H16].status == CheckStatus.BLOCKED
    assert results[WALL_GEOM_BODY_THICKNESS_GE_250].status == CheckStatus.BLOCKED
    assert results[WALL_GEOM_BODY_THICKNESS_GE_H16].evaluation_level == EvaluationLevel.NO_DATA
    assert results[WALL_GEOM_BODY_THICKNESS_GE_250].evaluation_level == EvaluationLevel.NO_DATA

    _, _, tall_run = _run(tables=_tables(story_height=6.0))
    restrained = next(
        result for result in tall_run.check_results if result.check_id == WALL_GEOM_RESTRAINED_LEG_THICKNESS
    )
...
def test_lw_bw_failure_does_not_delete_wall_inventory_identity():
    ...
    result = next(result for result in run_wall(snapshots) if result.check_id == WALL_GEOM_DEFINITION_LW_BW_GE6)
    assert result.status == CheckStatus.FAIL
    assert result.component == wall_id
...
def test_unrestrained_length_is_never_substituted_from_total_wall_length():
    def context(_record):
        data = _provided_context()
        data.pop("unrestrained_plan_length_mm")
        return data

    _, snapshots = _snapshots(context_factory=context)
    assert snapshots[0].features["wall_length_mm"].status == FeatureValueStatus.RESOLVED
    result = next(
        result for result in run_wall(snapshots) if result.check_id == WALL_GEOM_UNRESTRAINED_THICKNESS_GE_L30
    )
    assert result.status == CheckStatus.BLOCKED
...
def test_all_five_emit_canonical_checkresults_and_assessment_stays_incomplete():
    _, _, run = _run()
    assert len(run.check_results) == 5
    assert all(isinstance(result, CheckResult) for result in run.check_results)
    assert run.assessment.status_counts[CheckStatus.OK.value] == 3
    assert run.assessment.status_counts[CheckStatus.BLOCKED.value] == 2
    assert run.assessment.pack_a_status == "INCOMPLETE"
...
def test_reporter_is_dumb_and_preserves_canonical_result_without_recomputing():
    synthetic = CheckResult(
        check_id=WALL_GEOM_BODY_THICKNESS_GE_250,
        component="wall-x",
        component_type="wall",
    ...
    assert payload["results"][0]["limit"] == 9999.0
