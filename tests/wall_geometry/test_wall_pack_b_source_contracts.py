...
from tbdy_engine.features.result_evidence import (
    BASE_REACTION_IDENTITY_FIELDS, PIER_FORCE_IDENTITY_FIELDS, STORY_FORCE_IDENTITY_FIELDS,
)
...
def test_live_result_source_overlay_promotes_exact_full_identities():
    raw = yaml.safe_load(OVERLAY.read_text(encoding="utf-8"))
    tables = raw["tables"]
    expected = {
        "base_reactions": (BASE_REACTION_IDENTITY_FIELDS, 96, "Base Reactions"),
        "pier_forces": (PIER_FORCE_IDENTITY_FIELDS, 4416, "Pier Forces"),
        "story_forces": (STORY_FORCE_IDENTITY_FIELDS, 392, "Story Forces"),
    }
    for key, (fields, rows, live_name) in expected.items():
        item = tables[key]
        assert item["evidence_status"] == "VERIFIED_LIVE"
        assert item["live_table_name"] == live_name
        assert tuple(item["required_columns"]) == fields
