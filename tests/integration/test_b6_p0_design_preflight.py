...
class _FakeAnalysisLineage:
    def __init__(self, source_model_ref: str) -> None:
...
def harness(monkeypatch):
    ...
    def identity(_session):
        return SimpleNamespace(model_full_path=state["active_path"])

    def availability(_session):
        state["calls"].append(("availability", None))
    ...
    def summary(_session, frame_name):
        state["calls"].append(("summary", frame_name))
