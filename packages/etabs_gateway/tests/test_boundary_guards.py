...
def test_metadata_reads_are_scoped_to_context_reader() -> None:
    ...
    for path in iter_source_files():
        ...
        for node in ast.walk(tree):
            ...
            # Direct metadata API calls are allowed only in context_reader.py.
            ...
            # context_reader uses the shared _invoke(target, "MethodName", ...)
            # boundary, so inspect its literal method-name argument.
